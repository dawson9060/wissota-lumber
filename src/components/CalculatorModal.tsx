     ...
     <>
      <Modal opened={opened} onClose={close} title="Board-Foot Calculator">
        <Stack>
            ...
            <Text fz=".925rem">
                A board foot is a measure of volume. One board foot of lumber is one square foot that is one-inch thick.
            </Text>
            <Text fz=".925rem">Formula (in inches): (length x width x thickness) / 144</Text>
        </Stack>
         <Stack gap="0.5rem" mt="1rem">
            <NumberInput label="Width (inches)"       withAsterisk
      hideControls value={width} onChange={setWidth} />
            <NumberInput label="Length (inches)"       withAsterisk
      hideControls value={length} onChange={setLength} />
            <NumberInput label="Thickness (inches)"       withAsterisk
      hideControls value={thickness} onChange={setThickness} />
          ...
          <Group mt="1rem" bg="ghostwhite" p="1rem" justify='space-between'>
            <Text><strong>Total Board-Feet: </strong>{totalBoardFeet}</Text>
            <Divider orientation='vertical' />
            <Text><strong>Total Price: $</strong>{price && totalBoardFeet ? totalBoardFeet * Number(price) : '0'}</Text>
          </Group>
            <Group justify="flex-end" mt="2rem" gap="0.5rem">
                <Button color="orange.5" variant="outline" onClick={handleReset}>Reset</Button>
                <Button color="blue.5" onClick={close}>Close</Button>
            </Group>
      </Modal>

      <Text className={classes.links} onClick={open}>Calculator</Text>
    </>
  )
};
